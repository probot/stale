...
const fields = {
  ...
  daysUntilClose: Joi.alternatives().try(Joi.number(), Joi.boolean().only(false))
    .error(() => '"daysUntilLock" must be a number or false')
    .description('Number of days of inactivity before a stale Issue or Pull Request is locked. If disabled, issues still need to be locked manually, but will remain marked as stale and closed.'),
  ...
  lockComment: Joi.alternatives().try(Joi.string(), Joi.boolean().only(false))
    .error(() => '"lockComment" must be a string or false')
    .description('Comment to post when locking a stale Issue or Pull Request. Set to `false` to disable'),
...
const schema = Joi.object().keys({
  ...
  daysUntilLock: fields.daysUntilLock.default(7),
  ...
  lockComment: fields.lockComment.default(false),
