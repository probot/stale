...
module.exports = class Stale {
  constructor(github, config) {
    ...
    this.config = Object.assign({
      ttl: 1000 * 60 * 5, //60 * 24 * 60 * 60 * 100, // 60 days
  ...
  async getIssues() {
    ...
    const params = {
      q: `repo:${this.config.owner}/${this.config.repo} is:issue is:open updated:<${timestamp}`,
      sort: 'updated',
      order: 'desc',
      per_page: 100
    }

    console.log("searching for stale issues", params);
  ...
  mark(issue) {
    ...
    if (perform) {
      console.log("%s/%s#%d is being marked", owner, repo, number);
      ...
      console.log("%s/%s#%d would have been marked (dry-run)", owner, repo, number);
    }
  }
  ...
  close(issue) {
    ...
    if (perform) {
      console.log("%s/%s#%d is being closed", owner, repo, number);
      ...
      console.log("%s/%s#%d would have been closed (dry-run)", owner, repo, number);
    }
  }
  ...
  unmark(issue) {
    ...
    if (perform) {
      console.log("%s/%s#%d is being unmarked", owner, repo, number);
      return this.github.issues.removeLabel({owner, repo, number, name: staleLabel});
    } else {
      console.log("%s/%s#%d would have been unmarked (dry-run)", owner, repo, number);
    }
  }
