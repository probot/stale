...
module.exports = class Stale {
  ...
  async markAndSweep (type) {
    ...
    const {daysUntilLock, daysUntilClose, owner, repo} = this.config

    if (daysUntilLock) {
      this.getLockableIssues(type).then(res => {
        res.data.items.filter(issue => !issue.locked)
          .forEach(issue => this.lock(type, issue))
      })
    } else {
      this.logger.trace({owner, repo}, 'Configured to not lock stale issues')
    }
  ...
  getLockableIssues (type) {
    const staleLabel = this.getConfigValue(type, 'staleLabel')
    const queryTypeRestriction = Stale.getQueryTypeRestriction(type)
    const query = `label:"${staleLabel}" ${queryTypeRestriction} is:closed`
    const days = this.getConfigValue(type, 'days') || this.getConfigValue(type, 'daysUntilClose')
    return this.search(type, days, query)
  }
  ...
  async lock (type, issue) {
    ...
    if (perform) {
      ...
      if (lockComment) {
        await this.github.issues.createComment({owner, repo, number, body: lockComment})
      }
