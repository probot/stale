...
module.exports = class Stale {
  ...
  async markAndSweep (type) {
    ...
    const {closeStale, owner, repo} = this.config
    if (closeStale) {
      this.logger.info('%s/%s is configured to close stale issues', owner, repo)
      this.getClosable(type).then(res => {
        res.data.items.filter(issue => !issue.locked)
          .forEach(issue => this.close(type, issue))
      })
    } else {
      this.logger.info('%s/%s is configured to leave stale issues open', owner, repo)
    }
