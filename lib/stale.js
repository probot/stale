...
module.exports = class Stale {
  ...
  getConfigValue2(type,key){
	...
	for(var j=0;j<this.config["exemptLabels"].length;j++)
	{
		console.log(this.config["exemptLabels"][j])
		console.log(this.github.issues.getLabel({owner, repo, name:this.config["exemptLabels"][j]}))
...
//check for label in the labels array 
    for(var i=0;i<this.config["Labels"].length;i++)
    {
    	...
    	console.log(this.config["Labels"][i])
    	console.log("KEYS:"+Object.keys(this.config["Labels"][i]))
    	console.log(this.github.issues.getLabel({owner, repo, name:Object.keys(this.config["Labels"][i])}))
    	console.log(this.config["Labels"][label][i][key])
    	if (this.github.issues.getLabel({owner, repo, name:Object.keys(this.config["Labels"][i])}))
    	{
  			if (this.config["Labels"][i] && typeof this.config["Labels"][label][i][key] !== 'undefined'){
  				val = min(val,this.config["Labels"][label][i][key])
  			}
  		}
  	}
    if (val<100000)
